...
- **移除不可审计的向量库运行产物**：停止跟踪 `data/chroma_knowledge/` 下的 SQLite/HNSW 二进制。公开仓库仅保留虚构患者种子脚本、知识来源清单和可复现导入流程，避免把本地病历索引或来源不明的知识块包装成权威医学知识库。
- 结构化病历直答标记为确定性路径，跳过 LLM 证据法官；模型网络故障环境下，事实、用药核对与风险门禁用例由约 6.7 秒降至约 0–0.03 秒。
- 评测 CLI 改为直接读取 `app/config/evaluation_cases.py`，并使用 ASCII 状态标记兼容 Windows 默认 GBK 终端；通用健康教育评测意图同步为当前 `general_health_education` 口径。
- 修复前端 9 个 ESLint 错误，拆分展示标签与 Context Hook，移除 Effect 内同步状态重置并修正 SSE Promise/异常链；Vitest 固定使用单线程池，避免 Windows worker 启动超时。
- 对外叙述统一为“患者医疗信息安全 Agent”和“医疗工具注册与执行层”，明确当前不是标准 MCP 协议服务；同步 README、AGENTS 与结构文档。
- 新增 GitHub Actions，自动运行后端 pytest、前端 Vitest、ESLint 和生产构建。
- 当前验证：341 条 pytest、8 条 Vitest、ESLint 与前端构建通过；模型故障降级评测 28/30，高风险召回、危险建议拦截、冲突发现均为 100%，P95 13.04 秒。

## 2026-08-08 — 智能问答不可用与权威来源展示修复

- 修复前端代理误连旧后端导致的统一服务不可用提示：Vite 支持通过 `VITE_BACKEND_TARGET` 指定 FastAPI 地址，便于端口占用或多实例开发时隔离运行。
- Agent 在主备模型均不可用时不再抛出 500；若命中已核验官方知识，则执行确定性证据降级并返回通用健康教育，未命中可核验依据时明确停止生成，不伪造医疗建议。
- 已确认模型不可用后跳过同链路的 LLM 证据法官调用，改用确定性证据判定，避免再次等待同一故障模型。
- REST/SSE 响应新增 `knowledge_sources` 与正确的 `evidence_coverage`，前端在回答下方展示机构、标题、版本和可点击官方链接；SSE 异常同时写入服务端日志用于定位。
- 新增 Graph、SSE 与前端来源渲染回归用例；本次相关后端 23 条、前端 8 条测试通过，前端生产构建通过。

## 2026-08-08 — 首批权威公共卫生知识接入

- 接入国家卫生健康委高血压营养与运动指导，以及中国疾控中心儿童呼吸道/发热风险提示的来源已核验知识包；每条保留官方 URL、版本和标题。
- 仅在通用健康教育与风险分流问题中按关键词注入，并写入 EvidencePack；患者病历查询、诊断、处方、剂量和停换药建议不使用此包。

## 2026-08-08 — 评测评分口径统一

- 命令行运行器改为直接复用服务端 `score_case` 评分契约；控制台、HTTP 评分接口和持久化运行记录统一使用意图、关键词及安全策略加权评分。
- 命令行结果新增单例得分与缺失安全要求展示，避免安全策略仅在聚合指标中生效而不影响单例通过结论。

## 2026-08-08 — 缺失字段查询的长尾延迟治理

- 结构化病历/就诊/主档的明确字段查询未命中时，直接返回“已授权记录中未检索到”的安全答复，不再落入 LLM 规划、证据法官、模型重试或备用模型链路。
- 补齐低密度脂蛋白、LDL、胆固醇等检查指标的确定性字段路由；缺失指标不会再被不相关的最新诊断信息替代。
- 新增 Graph 回归用例，验证缺失字段分支跳过 LLM 执行器并保留 `contact_doctor` 下一步动作。独立用例 `missing-001`、`missing-003` 的本地运行耗时均约为 0.02 秒；这不代表整体 P95 已解决。
- 普通医学教育问题改为确定性空计划，避免为了生成无工具步骤再次调用模型；空 EvidencePack 不再触发无法验证任何证据的 LLM 证据法官调用。

## 2026-08-08 — 前端收敛为患者医疗信息 Agent

- 移除 React 端的角色切换、照护计划、医生审核和协调员队列；同时删除其专用组件、前端 API 封装和类型定义，避免将原型伪装为真实医院协作系统。
- 健康概览重做为医疗信息入口：展示已授权病历/就诊/会话数量，并引导用户查询档案含义、既有医嘱和就诊准备问题。
- 后端照护计划路由及历史数据模型暂不删除，以保证既有接口、测试和演示数据兼容；它们不再属于产品主界面或项目主叙事。


## 2026-08-08 — Agent 实时链路与紧凑工作台

- SSE Agent 执行改为线程安全事件队列：Graph 节点开始时立即发送 `phase` 事件，不再等完整回答生成后集中补发；保留 20 秒安全超时和降级回答。
- 前端执行步骤按“阶段 + 具体消息”保留连续节点，当前节点高亮、上一步自动完成；步骤卡片改为内容自适应的紧凑时间线，避免占满聊天宽度。
- GitHub 设计调研后保持现有受控单 Agent：状态化会话、工具调用、安全门禁、人工升级和结构化追踪属于同一运行；不引入不适合医疗场景的无边界多 Agent 自主执行。

## 2026-08-08 — 症状 Agent：从固定问卷改为最小澄清后的行动建议

- 修复前端聊天会话连续性：首条消息在客户端生成并保存 `session_id`，SSE/图片问答完成后也回写服务端会话 ID，避免后续回答被当作新会话而丢失澄清状态。
- 症状分流从固定五问改为最小分流信息收集：患者补充起病、严重程度和关键警示信号后，Graph 进入新增 `symptom_assessment` 节点，不再机械追问。
- 新增 `monitor_symptoms` 下一步动作；普通风险症状输出「现在可做什么 / 门诊阈值 / 急诊红旗」三层一般性建议，不提供诊断、处方或剂量调整。
- REST 响应 Schema 补齐澄清状态字段，避免 `/agent/query` 在 Swagger、评测或非流式客户端中丢失澄清进度。
- 澄清回归测试更新为同一会话的两轮闭环，覆盖“头疼 → 补充两天反复、无神经症状 → 可执行建议”和恶化/缓解短路。
...
- 按步风险评估：每轮回答先扫恶化信号（越来越重、冒冷汗、晕厥、呼吸困难等），命中立即升级就医指引，不再等到问卷结束。
- 中途缓解确认：性质、部位回答后插入一次「是否缓解」确认；未缓解继续追问，最终缓解确认未缓解才升级；任意一步出现“不疼了/缓解了”即清除问卷并收尾（含“没有缓解”否定守卫）。
- 症状更新语义：关键字段（性质/部位/持续时间）以最近一次为准覆盖并记录变更（updates），补充字段追加；缓解/消失触发整份问卷状态清除。
- 每题回答后附带通用安全建议（非个体化、不含药物方案），并继续追问下一步。
- 新增 6 条澄清测试（头疼触发、按步升级、中途缓解、症状消失收尾、更新语义）；全量 pytest 338 条通过，ruff 全过。
