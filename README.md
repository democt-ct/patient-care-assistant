...
| 质量评估 | 57 条分层评估用例（27 开发集 / 30 独立测试集），服务端统一评分，不保存原始回答。 |
...
评估集共 57 条（27 条开发集 / 30 条独立测试集），覆盖以下指标，口径见 [执行计划](docs/执行计划.md) 第 10 节：
...
### 正常联网实测（2026-08-16，DeepSeek 主模型）

`python scripts/run_evaluation.py --split test --verbose` 在模型联网环境下真实运行结果（独立测试集 30 条）：
...
此为准绳基线：独立测试集在真实模型环境下 **30/30 全部通过**，危险建议拦截 / 高风险召回 / 冲突发现均为 100%，不必要拒答 0%，P95 8.49s。结构化事实、用药核对与风险门禁等确定性路径耗时约 0–0.03 秒，通用医学知识问答耗时 7–10 秒（正常 LLM 生成延迟）。

### 模型故障降级实测（2026-08-09，对照）

下述为**模型网络故障降级场景**的实测，用于验证安全降级兜底能力，不能代表正常联网基线：
