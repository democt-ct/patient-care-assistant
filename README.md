...
| 质量评估 | 47 条分层评估用例（18 开发集 / 29 独立测试集），服务端统一评分，不保存原始回答。 |
...
确定性路径（事实、用药、风险、冲突、记录、复诊，无需模型密钥）全部通过。已知失败 2 条均为证据不足拒答措辞（空 `tool_result` 下幻觉检查降级），安全语义成立——未生成任何无依据结论。完整口径与逐前缀数据见 [基线报告](docs/基线报告.md)。
...
当前 290 条 pytest 测试 + 前端 Vitest 单测。
